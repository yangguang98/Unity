...
/// 控制层：射箭手AI
/// </summary>
public class ArcherAICommand : Command {

    float rotateSpeed = 1f;         //旋转速度
    public float moveSpeed = 5f;    //移动速度
    public float attackDistance = 15f;   //攻击距离
    public float cordonDistance = 20f;   //警戒距离
    public float thinkInterval = 3f;    //思考的间隔时间
    ...
    void Start()
    {
        ...
        //确定个体差异性参数
        moveSpeed = UnityHelper.GetInstance().GetRandomNum(1, 3);
        attackDistance = UnityHelper.GetInstance().GetRandomNum(11, 15);
        cordonDistance = UnityHelper.GetInstance().GetRandomNum(16, 25);
        thinkInterval = UnityHelper.GetInstance().GetRandomNum(2, 4);
